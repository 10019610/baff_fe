...
import { Trophy, Calendar, Target, Zap, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function ActiveBattles() {
  ...
  useEffect(() => {
    ...
    if (savedBattles) {
      ...
      localStorage.setItem('weightBattles', JSON.stringify(initialBattles));
    }
  }, []);
  ...
  const updateBattleProgress = (battle: Battle) => {
    ...
    const daysElapsed = Math.floor((Date.now() - new Date(battle.startDate).getTime()) / (1000 * 60 * 60 * 24));
    const totalDays = Math.floor((new Date(battle.endDate).getTime() - new Date(battle.startDate).getTime()) / (1000 * 60 * 60 * 24));
    const progressRatio = Math.min(daysElapsed / totalDays, 1);
    
    const opponentEfficiency = 0.8 + Math.random() * 0.4;
    const opponentCurrentWeight = battle.opponentStartWeight - (battle.targetWeightLoss * progressRatio * opponentEfficiency);
  ...
  const calculateOpponentProgress = (battle: Battle) => {
    ...
    const weightLoss = battle.opponentStartWeight - battle.opponentCurrentWeight;
    return Math.min((weightLoss / battle.targetWeightLoss) * 100, 100);
  };
    ...
    return (
      <Card>
        <CardContent className="pt-12 pb-12 text-center">
          ...
          <h3 className="text-xl font-medium mb-3">진행 중인 대결이 없습니다</h3>
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {activeBattles.filter(battle => getBattleWinner(battle) === 'me').length}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round(activeBattles.reduce((acc, battle) => acc + calculateMyProgress(battle), 0) / activeBattles.length)}%
                </p>
              </div>
      ...
      <div className="space-y-6">
        {activeBattles.map(battle => {
          ...
          const myWeightLoss = battle.myCurrentWeight ? battle.myStartWeight - battle.myCurrentWeight : 0;
          const opponentWeightLoss = battle.opponentCurrentWeight ? battle.opponentStartWeight - battle.opponentCurrentWeight : 0;
            ...
            <Card key={battle.id} className="overflow-hidden">
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge 
                    variant={winner === 'me' ? 'default' : winner === 'opponent' ? 'destructive' : 'secondary'}
                    className="px-3 py-1"
                  >
                    {winner === 'me' ? '🏆 리드 중' : winner === 'opponent' ? '😤 뒤쳐짐' : '🤝 접전'}
                  </Badge>
                </div>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-center p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                      <div>
                        <p className="text-lg font-bold text-blue-600">{myWeightLoss.toFixed(1)}kg</p>
                        <p className="text-xs text-muted-foreground">감량</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-blue-600">{(battle.targetWeightLoss - myWeightLoss).toFixed(1)}kg</p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-center p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg">
                      <div>
                        <p className="text-lg font-bold text-orange-600">{opponentWeightLoss.toFixed(1)}kg</p>
                        <p className="text-xs text-muted-foreground">감량</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-orange-600">{(battle.targetWeightLoss - opponentWeightLoss).toFixed(1)}kg</p>
                        <p className="text-xs text-muted-foreground">목표까지</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-lg font-medium">{battle.targetWeightLoss}kg</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">차이</p>
                    <p className={`text-lg font-medium ${Math.abs(myProgress - opponentProgress) < 5 ? 'text-orange-500' : myProgress > opponentProgress ? 'text-green-500' : 'text-red-500'}`}>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-medium">
                      {Math.ceil((new Date(battle.endDate).getTime() - new Date(battle.startDate).getTime()) / (1000 * 60 * 60 * 24))}일
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">남은 시간</p>
                    <p className={`text-lg font-medium ${daysRemaining <= 3 ? 'text-red-500' : daysRemaining <= 7 ? 'text-orange-500' : 'text-green-500'}`}>
                      {daysRemaining}일
                    </p>
                  </div>
                </div>
                ...
                <div className={`p-4 rounded-lg text-center ${
                  ...
                  <p className={`text-sm font-medium ${
                    winner === 'me' ? 'text-green-700 dark:text-green-300' :
                    winner === 'opponent' ? 'text-red-700 dark:text-red-300' :
