...
export const adminApi = {
  ...
  getUserGrowth: (period = 'DAILY') =>
    api.get(`${BASE}/user-growth`, { params: { period } }),
  getWeightTrend: (period = 'DAILY') =>
    api.get(`${BASE}/weight-trend`, { params: { period } }),
  getPlatformDistribution: () => api.get(`${BASE}/platform-distribution`),
  getRecentActivities: () => api.get(`${BASE}/recent-activities`),

  // S6-16 — 그램경제 스냅샷
  getGramEconomySnapshot: () => api.get(`${BASE}/gram-economy/snapshot`),
  ...
  updateUserRole: (userId: number, role: string) =>
    api.put(`${BASE}/users/${userId}/role`, { role }),
  updateUserStatus: (userId: number, status: string) =>
    api.put(`${BASE}/users/${userId}/status`, { status }),
  ...
  replyToInquiry: (id: number, content: string) =>
    api.post(`${BASE}/inquiries/${id}/reply`, { content }),
  updateInquiryStatus: (id: number, status: string) =>
    api.put(`${BASE}/inquiries/${id}/status`, { status }),
  ...
  updateReviewVisibility: (id: number, isPublic: boolean) =>
    api.put(`${BASE}/reviews/${id}/visibility`, { isPublic }),
  ...
  // 내역 관리
  getLoginHistories: (params: {
    page?: number;
    size?: number;
    userId?: number;
  }) => api.get(`${BASE}/history/logins`, { params }),
  getWeightHistories: (params: {
    page?: number;
    size?: number;
    userId?: number;
  }) => api.get(`${BASE}/history/weights`, { params }),
  getRewardHistories: (params: {
    type?: string;
    page?: number;
    size?: number;
  ...
  updateTossAdConfig: (
    position: string,
    data: import('../../types/Admin.api.type.ts').UpdateTossAdConfigRequest
  ) => api.put(`${BASE}/toss-ad/configs/${position}`, data),

  // 공지사항
  getNotices: () => api.get(`${BASE}/notices`),
  createNotice: (title: string, content: string) =>
    api.post(`${BASE}/notices`, { title, content }),
  updateNotice: (
    id: number,
